...
export function WikipediaMacroPanel({ onStatusChange }: WikipediaMacroPanelProps) {
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (result.error && !result.data) {
        ...
        // Se há erro mas temos dados (provavelmente do cache), mostramos os dados e o erro.
        setConflictsData(result.data);
        setError(result.error); // Mostrar erro como aviso, mas ainda renderizar dados do cache
        ...
        // Nenhum dado e nenhum erro (ex: AI não encontrou conflitos)
        ...
        // Sucesso e temos dados
        setConflictsData(result.data || null);
        onStatusChange({ status: 'success' });
      }

    } catch (errCaught) { // Este catch é para erros inesperados no próprio fetchData ou actions
  ...
  // Se temos dados, mesmo que haja um erro (que será exibido como aviso), mostramos os dados.
  ...
  return (
    <div className="flex flex-col">
      ...
      {error && conflictsData && conflictsData.conflicts.length > 0 && ( // Mostra erro como aviso se temos dados
      ...
      {(!isLoading || isRefreshing) && conflictsData && conflictsData.conflicts && conflictsData.conflicts.length > 0 && (
        <>
          <div className="mb-6 h-[500px] md:h-[600px] w-full"> {/* Aumenta a altura do container do mapa */}
             <MapDisplay
                key={conflictsData?.lastUpdated || 'map-initial-loading-state'} // Re-render map on data update
            ...
            {severityOrder.map((severityKey) => {
              ...
              return (
                <AccordionItem value={severityKey} key={severityKey}>
                  ...
                  <AccordionContent>
                    <div className="space-y-4"> {/* Aumentado space-y para melhor separação dos cards com imagem */}
                      {conflicts.map((conflict) => (
                        <div key={conflict.id} className="p-4 border rounded-lg shadow-sm bg-card hover:shadow-lg transition-shadow">
                          <div className="relative w-full h-48 mb-3 rounded-md overflow-hidden"> {/* Container para a imagem */}
                            <Image
                              ...
                              onError={(e) => {
                                // Em caso de erro ao carregar a imagem da Wikipedia, usa um placeholder

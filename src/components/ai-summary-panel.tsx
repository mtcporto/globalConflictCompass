...
import type { SourceStatus, SummarizeNewsInputItem, BbcNewsItemRss, ReliefWebReport } from '@/lib/types'; // Added BbcNewsItemRss, ReliefWebReport
    ...
    try {
      ...
      bbcData.forEach((item: BbcNewsItemRss) => {
        newsToSummarize.push({
          title: item.title,
          description: item.description.replace(/<[^>]*>?/gm, '').substring(0, 350) + '...', // Slightly longer snippet
          link: item.link,
        });
      });
    ...
    <div className="flex flex-col h-full">
      ...
      {summary && !isLoading && (
        <ScrollArea className="flex-grow">
          <div className="p-3 bg-card rounded-lg shadow-md space-y-4 text-sm">
            {summary.resumoGeral && (
              <div>
            ...
            {summary.principaisZonasDeConflito && summary.principaisZonasDeConflito.length > 0 && (
              <div>
                <h4 className="font-semibold text-base mb-1 text-accent">Principais Zonas de Conflito:</h4>
                <ul className="list-disc list-inside ml-4 space-y-0.5 text-foreground/90">
                  {summary.principaisZonasDeConflito.map((zona, index) => <li key={`zona-${index}`}>{zona}</li>)}
                </ul>
              </div>
            )}
      ...
      {!summary && !isLoading && !error && (
         <p className="text-sm text-muted-foreground text-center flex-grow flex items-center justify-center">
           Clique no botão acima para gerar uma análise detalhada das últimas notícias de conflito.
         </p>
      )}
    </div>
  );
